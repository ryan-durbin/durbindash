...
Deploy DurbinDash as a systemd service so it runs automatically on boot and restarts on failure.
...
2. Copy `durbindash.service` to `/etc/systemd/system/`
...
systemctl status durbindash

# Stop the service
sudo systemctl stop durbindash

# Restart the service
sudo systemctl restart durbindash

# View logs
journalctl -u durbindash -f
```

### Automatic Restart

The service is configured to restart automatically on failure with a 5-second delay (`RestartSec=5`). This ensures DurbinDash recovers from unexpected crashes without manual intervention.
